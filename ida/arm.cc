  ...
  for (uint8_t operand_position = 0;
    ...
    switch (operand.type) {
      ...
      case o_mem: {
        ...
        const Name name = GetName(instruction.ea, immediate, operand_position, false);
        expressions.push_back(expression = Expression::Create(
      ...
      case o_displ: {
        ...
        if (operand.addr) {
          ...
          expressions.push_back(Expression::Create(
              ...
              0, Expression::TYPE_REGISTER, 0));
          if (offset) {
            const Name name = GetName(instruction.ea, offset, operand_position, false);
            expressions.push_back(Expression::Create(
        ...
        if (co_processor) {
          ...
          expressions.push_back(
              expression = Expression::Create(expression, ",", 0,
                                              Expression::TYPE_OPERATOR, 0));
          expressions.push_back(Expression::Create(
              expression, GetCoprocessorName(static_cast<size_t>(operand.specflag1)), 0,
      ...
      case o_idpspec5 + 1: {
        ...
        LOG(INFO) << StringPrintf(
            "warning: text storage not yet supported (%08" PRIx64 ")", instruction.ea);
        break;
      }
      default: {
        LOG(INFO) << StringPrintf(
            "warning: unknown operand type %d at %08" PRIx64,
            static_cast<int>(operand.type), instruction.ea);
        break;
      }
    }
